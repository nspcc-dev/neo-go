...
- `Timeout` is a timeout for a single request to NeoFS storage node (10 minutes by
  default).
- `ContainerID` is a container ID to fetch blocks from. If not specified, the
  container ID will be taken from the `NeoFSBlockFetcher` configuration section.
