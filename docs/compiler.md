...
`checkMethod` interprets `Standard.Parameters` as follows:

* `nil` — parameter count is unspecified by the standard. `checkMethod` does not
  enforce a specific number of parameters. Use `nil` when the standard allows an
  undefined number of parameters.
* non-nil slice — the standard requires an exact match of the parameter list.
