...
Header is used to mock the execution environment so that interops like
`System.Runtime.GetTime` and native Ledger contract's APIs like `currentHash`
or `currentIndex` work properly. It's up to the caller to decide which fields
of transaction and header should be filled in, but at least transaction's
script is required for successful request processing. If header is not specified,
then "fake" header will be generated on the server side based on the current
chain's height. If needed, transaction's hash and size and header's hash may be
overridden to mock transaction's hash/size and header's hash respectively,
no hash/size check is performed on the server side during deserialization. If
trigger parameter specified then it will be used to run the transaction's script
(`Application` is used by default). If `Verification` trigger is used then
execution GAS limit will be restricted by the current `MaxVerificationGAS` Policy
value. `OnPersist` and `PostPersist` triggers are not supported for now. If
verbose parameter specified then invocation tree data will be attached to the
result (similar to `invokefunction` behaviour, `false` is used as a default value).
All parameters are positional. Note that similar to `invokefunction`, this method
performs a test invocation and will not affect the blockchain state.
...
trigger-sensitive interops and native contract APIs work as expected. 
