...
#### SessionExpansionEnabled extension

`SessionExpansionEnabled` – When enabled, the RPC server returns only a partial expansion of any 
iterator returned by `invoke*` calls. Specifically, up to `MaxIteratorResultItems` elements are 
...
is also `true`, an iterator `ID` is provided allowing clients to retrieve additional elements via
...
If `SessionEnabled` is `false`, the iterator is expanded up to `MaxIteratorResultItems`, and 
additional values cannot be retrieved.
