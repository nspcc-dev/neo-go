...
func (bfs *Service) Start() error {
	...
	if err := bfs.finalizeState(syncHeight); err != nil {
...
func (bfs *Service) finalizeState(height uint32) error {
	...
	tempCache := storage.NewMemCachedStore(bfs.stateModule.Store)

	bfs.stateModule.Store.Seek(storage.SeekRange{
		Prefix: tempPrefix,
	}, func(k, v []byte) bool {
		newKey := append(permPrefix, k[1:]...)
		tempCache.Put(newKey, v)
		tempCache.Delete(k)
		return true
	})

	if n, err := tempCache.Persist(); err != nil {
