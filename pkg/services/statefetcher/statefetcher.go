...
// FindStateObject finds and caches the most recent state object within
// [min, max] height range. If max is 0, the latest available object is
...
//
// TODO: OK.
func (s *Service) FindStateObject(min uint32, max uint32) (uint32, error) {
	s.lock.RLock()
	if s.stateRoot.Index != 0 && s.stateRoot.Index >= min && (max == 0 || s.stateRoot.Index <= max) {
		h := s.stateRoot.Index
		s.lock.RUnlock()
		return h, nil
	}
	...
	filters.AddFilter(s.cfg.StateAttribute, fmt.Sprintf("%d", min), object.MatchNumGE)
	if max > 0 {
		filters.AddFilter(s.cfg.StateAttribute, fmt.Sprintf("%d", max), object.MatchNumLE)
	}
...
loop:
	for {
		...
		case err = <-errs:
			if err != nil && !neofs.IsContextCanceledErr(err) {
				s.isActive.CompareAndSwap(true, false)
				return 0, fmt.Errorf("failed to search state object: %w", err)
			}
			break loop
		}
	}
	...
	if len(obj.Attributes) < 2 {
		s.isActive.CompareAndSwap(true, false)
		return 0, fmt.Errorf("missing state object attributes: expected at least 2, got %d", len(obj.Attributes))
	}
	h, err := strconv.ParseUint(obj.Attributes[0], 10, 32)
	if err != nil || h == 0 {
		s.isActive.CompareAndSwap(true, false)
		return 0, fmt.Errorf("invalid state object index: %w", err)
	}
	r.Index = uint32(h)
	...
	if err != nil {
		s.isActive.CompareAndSwap(true, false)
		return 0, fmt.Errorf("failed to decode state root from state object attribute: %w", err)
	}

	if len(obj.Attributes) > 2 {
		b, err := hex.DecodeString(obj.Attributes[2])
		if err != nil {
			s.isActive.CompareAndSwap(true, false)
	...
	s.log.Info("new state object found",
		zap.Stringer("oid", obj.ID),
	...
	defer s.lock.Unlock()
	s.stateRoot = *r
	s.stateObjectID = obj.ID

	return s.stateRoot.Index, nil
}
...
func (s *Service) run(syncPoint uint32) {
	...
	// Ensure the proper state object found before accessing its OID since the
	// node may be recovering after previously interrupted state sync at some
	// older state sync point.
	_, err := s.FindStateObject(s.chain.HeaderHeight()-1, syncPoint)
	if err != nil {
