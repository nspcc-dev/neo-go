...
func (s *Service) run(syncPoint uint32) {
	...
	// Ensure the proper state object found and cached by Service before
	// accessing its OID. The node may be recovering after previously
	// interrupted state sync at some older state sync point, hence the initial
	// state object found by Module may be too fresh for the StateSync module.
