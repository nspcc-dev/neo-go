...
func TestParseAppCallContract(t *testing.T) {
	check := func(t *testing.T, reveseitems bool) {
		...
		if reveseitems {
			emit.Opcodes(b.BinWriter, opcode.REVERSEITEMS)
		}
	...
	t.Run("all arguments present", func(t *testing.T) {
		...
		_, _, err := ParseAppCallContract(b.Bytes())
		require.ErrorContains(t, err, "at least one missing AppCall argument is required, got 2 out of 2")
	})
}
