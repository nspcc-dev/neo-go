...
func verifyIncompleteStandardInvocationScript(inv []byte) error {
	if len(inv) != 0 {
		if len(inv) != 66 || !bytes.HasPrefix(inv, []byte{byte(opcode.PUSHDATA1), keys.SignatureLen}) {
...
func ParseAppCallContract(script []byte) (int, int, error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("failed to parse AppCall: %w", err)
	}
	...
	if m == n {
		return 0, 0, fmt.Errorf("at least one missing AppCall argument is required, got %d out of %d", m, n)
	}
