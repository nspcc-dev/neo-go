...
func TestClient_InvokeContainedScript(t *testing.T) {
	...
	// TODO: @roman-khimov, pay attention that if we're prohibiting to use OnPersist/PostPersist triggers
	// with `invokecontainedscript` then it's not possible to retrieve persisting block hash from the contract,
	// hence hash mocking functionality is not needed for block.
	src.Reset()
