...
func (v *VM) execute(ctx *Context, op opcode.Opcode, parameter []byte, m map[opcode.Opcode]*S) (err error) {
	...
	case opcode.CONVERT:
		...
		} else if fromType == stackitem.ByteArrayT && toType == stackitem.BufferT {
	...
	case opcode.SETITEM:
		...
		args = []any{int(r1 - r2), int(r2 - r3), int(v.refs - r3), i + 1}
	...
	case opcode.VALUES:
		...
		r2 := v.refs
		v.estack.PushItem(stackitem.NewArray(arr))
		args = []any{int(r1 - r2), int(v.refs - r2), nClonedItems, len(arr)}
