...
func (v *VM) execute(ctx *Context, op opcode.Opcode, parameter []byte/*, m map[opcode.Opcode]*S*/) (err error) {
	...
	case opcode.PACKMAP:
		...
		m.IncRC()
		args = []any{int(v.refs - r), numComparisons}
	...
	case opcode.SETITEM:
		...
		case *stackitem.Array, *stackitem.Struct:
			...
			if index < 0 || index >= len(arr) {
				...
				args = []any{int(r - v.refs), nClonedItems, 0}
				v.throw(stackitem.NewByteArray([]byte(msg)))
				return
			}
		...
		args = []any{int(r - v.refs), nClonedItems, i + 1}
