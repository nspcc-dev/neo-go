...
func NewWithTrigger(t trigger.Type) *VM {
	...
	// Use OpcodeExecutorV1 because isHardforkEnabled(config.HFHuyao) = true.
...
func (v *VM) GasConsumed() int64 {
	consumed := PicoGasToDatoshi(new(uint256.Int).Div(v.gasConsumed, uint256OpcodePriceMultiplier))
...
func (v *VM) addPicoGasInternal(gas *uint256.Int) error {
	return v.AddFemtoGas(new(uint256.Int).Mul(gas, uint256OpcodePriceMultiplier))
}

// AddFemtoGas consumes the specified amount of gas in femtoGAS units if the
// executing method is not whitelisted. It returns [ErrGASLimitExceeded] if the
// gas limit was exceeded.
func (v *VM) AddFemtoGas(gas *uint256.Int) error {
...
func (v *VM) OpcodeExecutorV1(ctx *Context, op opcode.Opcode, parameter []byte) (err error) {
	...
	defer func(canGetPrice bool) {
		if canGetPrice {
			...
			p := v.getPrice(op, parameter, priceParams)
			if addErr := v.AddFemtoGas(uint256.NewInt(uint64(p))); addErr != nil {
				v.state = vmstate.Fault
				err = newError(ctx.IP(), op, addErr)
				return
			}
		}
		if errRecover := recover(); errRecover != nil {
