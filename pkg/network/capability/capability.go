...
// Capability describes the network service available for the node.
...
// Node represents full node capability with the start height.
type Node struct {
	StartHeight uint32
}
...
// Server represents TCP or WS server capability with the port.
type Server struct {
