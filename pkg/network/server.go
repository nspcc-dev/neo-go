...
func newServerFromConstructors(config ServerConfig, chain Ledger, stSync StateSync, log *zap.Logger,
	...
	var err error
	s.blockFetcher, err = blockfetcher.New(chain, s.NeoFSBlockFetcherCfg, log, func(item any) error { return s.bFetcherQueue.Put(item.(*block.Block)) }, sync.OnceFunc(func() { close(s.blockFetcherFin) }), blockfetcher.Blocks)
	if err != nil {
	...
	s.syncHeaderFetcher, err = blockfetcher.New(s.stateSync, s.NeoFSBlockFetcherCfg, log, func(item any) error { return s.syncHFetcherQueue.Put(item.(*block.Header)) },
		func() {
	...
	s.syncBlockFetcher, err = blockfetcher.New(s.stateSync, s.NeoFSBlockFetcherCfg, log, func(item any) error { return s.syncBFetcherQueue.Put(item.(*block.Block)) },
		sync.OnceFunc(func() {
