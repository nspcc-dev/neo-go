...
func (s *Server) stateSyncCallBack() {
	...
	if needHeaders {
		...
		if !s.syncHeaderFetcher.IsShutdown() {
			height, err := s.syncStateFetcher.LatestStateObjectHeight()
			if err != nil {
				return
			}
			err = s.syncHeaderFetcher.Start(height)
			if err != nil {
	...
	if needBlocks {
		...
		if !s.syncBlockFetcher.IsShutdown() {
			...
			if err = s.syncBlockFetcher.Start(height); err != nil {
	...
	if !needHeaders && !needBlocks && !needStorage {
		s.syncBlockFetcher.Shutdown()
	}
	if !isActive {
...
func (s *Server) handleBlockCmd(p Peer, block *block.Block) error {
	if s.syncHeaderFetcher.IsActive() || s.syncBlockFetcher.IsActive() || s.blockFetcher.IsActive() || s.syncStateFetcher.IsActive() || s.stateSync.IsActive() && s.config.NeoFSStateSyncExtensions {
		return nil
	}
...
func (s *Server) tryInitStateSync() {
	...
	if !s.ServerConfig.NeoFSStateFetcherCfg.Enabled {
		s.lock.RLock()
		...
		h, err := s.syncStateFetcher.LatestStateObjectHeight()
		if err != nil {
