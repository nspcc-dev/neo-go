...
func (e *DesignationEvent) FromStackItem(item *stackitem.Array) error {
	...
	roleUint := roleBigInt.Uint64()
	if roleUint > math.MaxUint8 {
		return fmt.Errorf("role overflow: %d > %d", roleUint, math.MaxUint8)
	}
	e.Role = noderoles.Role(byte(roleUint))
	...
	blockIndexUint := blockIndexBigInt.Uint64()
	if blockIndexUint > math.MaxUint32 {
		return fmt.Errorf("blockIndex overflow: %d > %d", blockIndexUint, math.MaxUint32)
	}
	e.BlockIndex = uint32(blockIndexUint)
