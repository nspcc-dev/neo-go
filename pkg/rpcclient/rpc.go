...
// (if provided) is used to mock execution environment so that interops like
...
// use [transaction.SetHash], [transaction.SetSize] and [header.SetHash] methods
// to mock transaction's hash/size and header's hash respectively; other
// transaction's and header's fields may be mocked directly via value
// assignment. If trigger is specified then it will be used to run the
// transaction's script. If verbose is specified then invocation tree data will
// be attached to the result. All parameters are positional.
// NOTE: This is a test invoke and will not affect the blockchain.
