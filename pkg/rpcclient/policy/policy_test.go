...
func TestReader(t *testing.T) {
	...
	ta.res = &result.Invoke{
		Stack: []stackitem.Item{
			stackitem.Make(append(append(util.Uint160{1, 2, 3}.BytesBE(), []byte{0, 0, 0, 1}...), []byte("someMethod")...)),
		},
	}
	whitelisted, err := iter.Next(10)
	require.NoError(t, err)
	require.Equal(t, []WhitelistedContract{{Hash: util.Uint160{1, 2, 3}, Method: "someMethod", ArgCnt: 1}}, whitelisted)
}
...
func TestWhitelistChangedEvent_FromStackItem(t *testing.T) {
	tcs := []struct {
		name     string
		in       []stackitem.Item
		expected *WhitelistChangedEvent
		err      string
	}{
		{
			name: "nil item",
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			actual := new(WhitelistChangedEvent)
