...
// BlockedAccountsIterator is used for iterating over GetContractHashes results.
...
// GetBlockedAccounts returns current blocked accounts.
// Note that this method is available starting from Faun hardfork.
...
func (c *ContractReader) GetBlockedAccountsExpanded(num int) ([]util.Uint160, error) {
	arr, err := unwrap.Array(c.invoker.CallAndExpandIterator(Hash, "getBlockedAccounts", num))
	if err != nil {
		return nil, err
	}
	return itemsToHashes(arr)
}
...
func itemsToHashes(arr []stackitem.Item) ([]util.Uint160, error) {
