...
// WhitelistChangedEvent represents a WhitelistChanged Policy event.
type WhitelistChangedEvent struct {
	Hash   util.Uint160
	Method string
	ArgCnt int
	// Fee is the fixed execution cost of the method (it's negative if the
	// method is removed from the whitelist).
	Fee int64
}
...
func itemsToWhitelistedContracts(arr []stackitem.Item) ([]WhitelistedContract, error) {
	...
	for i, itm := range arr {
		...
		res[i].Hash = h
		res[i].ArgCnt = int(binary.BigEndian.Uint32(str[util.Uint160Size:]))
		res[i].Method = string(str[util.Uint160Size+4:])
	}
	return res, nil
}

// FromStackItem converts provided [stackitem.Array] to WhitelistChangedEvent or returns an
// error if it's not possible to do so.
func (e *WhitelistChangedEvent) FromStackItem(item *stackitem.Array) error {
	if item == nil {
