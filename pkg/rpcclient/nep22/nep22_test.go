...
func TestUpdateTransactionAndUnsigned(t *testing.T) {
	actor := new(testAct)
	tok := New(actor, util.Uint160{1, 2, 3})

	for name, fn := range map[string]func(nef, mf []byte, data any) (interface{}, error){
		"UpdateTransaction": func(nef, mf []byte, data any) (interface{}, error) {
			return tok.UpdateTransaction(nef, mf, data)
		},
		"UpdateUnsigned": func(nef, mf []byte, data any) (interface{}, error) {
