...
func getAssetMaps(name string) (map[util.Uint256]*result.AssetUTXO, map[util.Uint256]*result.AssetUTXO) {
...
func (s *Server) getUTXOTransfers(ps request.Params) (interface{}, *response.Error) {
	...
	sent, recv := getAssetMaps(assetName)
	lg := s.chain.(*core.Blockchain).GetTransferLog(addr)
	if lg != nil {
		tr := new(state.Transfer)
		err = lg.ForEach(state.TransferSize, tr, func() error {
			if tr.Timestamp < start || end != 0 && tr.Timestamp > end {
				return nil
			}
			a, ok := sent[tr.AssetID]
