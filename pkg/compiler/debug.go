...
func scAndVMInteropTypeFromExpr(named *types.Named, isPointer bool) (smartcontract.ParamType, stackitem.Type, binding.Override, *manifest.ExtendedType) {
	...
	case "ledger", "management":
		...
		typeName := pkg + "." + name
		et := &manifest.ExtendedType{Base: smartcontract.ArrayType, Name: typeName}
		if isPointer {
			typeName = "*" + typeName
		}
	...
	return smartcontract.InteropInterfaceType,
		...
		&manifest.ExtendedType{Base: smartcontract.InteropInterfaceType, Interface: "iterator"} // Temporarily all interops are iterators.
}
...
func (c *codegen) scAndVMTypeFromType(t types.Type, exts map[string]manifest.ExtendedType) (smartcontract.ParamType, stackitem.Type, binding.Override, *manifest.ExtendedType) {
	...
	case *types.Map:
		...
		et.Value = vet
		if et.Value == nil {
			et.Value = &manifest.ExtendedType{Base: vt}
		}
	...
	if exts != nil {
		if exts[name].Name != name {
			...
			for i := range et.Fields {
				...
				et.Fields[i].Type = ft
				if fet == nil {
					et.Fields[i].ExtendedType = &manifest.ExtendedType{Base: ft}
				} else {
...
func collectNamedTypes(et *manifest.ExtendedType, abiNamedTypes, opNamedTypes map[string]manifest.ExtendedType, mainPkg string) error {
	...
	if et.Name != "" {
		...
		if ext, ok = opNamedTypes[et.Name]; !ok {
			ext, ok = opNamedTypes[mainPkg+"."+et.Name]
		}
		if ok {
			if et.Fields == nil {
				if _, ok := abiNamedTypes[mainPkg+"."+et.Name]; !ok {
					abiNamedTypes[mainPkg+"."+et.Name] = ext
				}
				return nil
			}
			return fmt.Errorf("configured declared named type intersects with the contract's one: `%s`", mainPkg+"."+et.Name)
		}
...
func buildManifestExtendedType(dst, src *manifest.ExtendedType) {
	if src.Name != "" {
		dst.Name = src.Name
		dst.Base = smartcontract.ArrayType
		return
	}
	dst.Base = src.Base
