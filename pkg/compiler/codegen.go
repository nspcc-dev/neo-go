...
func (c *codegen) Visit(node ast.Node) ast.Visitor {
	...
	case *ast.GenDecl:
		...
		for _, spec := range n.Specs {
			...
			case *ast.ValueSpec:
				var isKeyCheck bool
				// Handle comma-ok form with two return values (type assertion or map index expression).
				if len(t.Names) == 2 && len(t.Values) == 1 && n.Tok == token.VAR {
					...
					isKeyCheck = c.checkGetMapValueWithOKFlag(t.Values[0])
				}
	...
	case *ast.AssignStmt:
		...
		if len(n.Lhs) == 2 && len(n.Rhs) == 1 && (n.Tok == token.DEFINE || n.Tok == token.ASSIGN) {
			...
			isKeyCheck = c.checkGetMapValueWithOKFlag(n.Rhs[0])
		}
