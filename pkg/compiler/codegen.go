...
func (c *codegen) convertBuiltin(expr *ast.CallExpr) {
	...
	case "min":
		for i, arg := range expr.Args {
			...
			if !ok {
				c.prog.Err = fmt.Errorf("min requires integer types")
				return
			}
			switch typ.Kind() {
			case types.Int, types.Int8, types.Int16, types.Int32, types.Int64,
				...
				types.UntypedInt:
			default:
				c.prog.Err = fmt.Errorf("min requires integer types, got at the %d position: %s", i+1, typ)
				return
			}
		}
	...
	case "max":
		for i, arg := range expr.Args {
			typ, ok := c.typeOf(arg).(*types.Basic)
			if !ok {
				c.prog.Err = fmt.Errorf("max requires integer types")
				return
			}
			switch typ.Kind() {
			case types.Int, types.Int8, types.Int16, types.Int32, types.Int64,
				...
				types.UntypedInt:
			default:
				c.prog.Err = fmt.Errorf("max requires integer types, got at the %d position: %s", i+1, typ)
				return
			}
		}
		for range len(expr.Args) - 1 {
