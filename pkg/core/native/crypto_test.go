...
func TestBlsPairingLst(t *testing.T) {
	...
	t.Run("success case", func(t *testing.T) {
		resGT := crypto.bls12381PairingLst(nil, []stackitem.Item{stackitem.NewArray([]stackitem.Item{
			stackitem.NewInterop(blsPoint{point: &g1Point}),
			stackitem.NewInterop(blsPoint{point: &g2Point}),
	...
	t.Run("errors", func(t *testing.T) {
		require.PanicsWithValue(t, "bls12381 pairing requires at least one pair", func() {
			crypto.bls12381PairingLst(nil, []stackitem.Item{stackitem.NewArray(nil)})
		})
		require.PanicsWithError(t, fmt.Sprintf("bls12381 pairing supports at most %d pairs", Bls12381PairingMaxPairs), func() {
			crypto.bls12381PairingLst(nil, []stackitem.Item{stackitem.NewArray(make([]stackitem.Item, Bls12381PairingMaxPairs+1))})
		})
		require.PanicsWithValue(t, "bls12381 pairing requires an even number of elements", func() {
			crypto.bls12381PairingLst(nil, []stackitem.Item{stackitem.NewArray([]stackitem.Item{nil})})
		})
		require.PanicsWithValue(t, "bls12381 pairing requires an even number of elements", func() {
			crypto.bls12381PairingLst(nil, []stackitem.Item{stackitem.NewArray([]stackitem.Item{nil})})
		})
		require.PanicsWithValue(t, "bls12381 pairing requires interop points", func() {
			crypto.bls12381PairingLst(nil, []stackitem.Item{stackitem.NewArray([]stackitem.Item{stackitem.Null{}, nil})})
		})
		require.PanicsWithValue(t, "interop must contain bls12381 point", func() {
			crypto.bls12381PairingLst(nil, []stackitem.Item{stackitem.NewArray([]stackitem.Item{
...
func TestBlsSerializersEthList(t *testing.T) {
	crypto := newCrypto()
	t.Run("errors", func(t *testing.T) {
		require.PanicsWithValue(t, "require at least one element", func() {
			crypto.bls12381DeserializeEthList(nil, []stackitem.Item{stackitem.NewArray(nil)})
		})
		g1Uncompressed := g1Point.RawBytes()
		require.PanicsWithValue(t, "invalid sequence of points", func() {
			crypto.bls12381DeserializeEthList(nil, []stackitem.Item{stackitem.NewArray([]stackitem.Item{
				stackitem.NewByteArray(toEthereum(g1Uncompressed[:])),
				stackitem.NewByteArray(toEthereum(g1Uncompressed[:])),
			})})
		})
