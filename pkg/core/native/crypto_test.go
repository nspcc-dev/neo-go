...
func TestBlsMultiExp(t *testing.T) {
	...
	for _, params := range []struct{ input, expected []byte }{
		...
		t.Run("eth g1 compat", func(t *testing.T) {
			pairs := crypto.bls12381DeserializeEthG1PointScalarPairs(nil, []stackitem.Item{
	...
	t.Run("eth g2 compat", func(t *testing.T) {
		pairs := crypto.bls12381DeserializeEthG2PointScalarPairs(nil, []stackitem.Item{
...
func TestBlsPairingLst(t *testing.T) {
	crypto := newCrypto()
	t.Run("eth compat", func(t *testing.T) {
		input := append(addPadding(g1Uncompressed[:]), g2ToEthereum(g2Uncompressed[:])...)
		points := crypto.bls12381DeserializeEthPoints(nil, []stackitem.Item{stackitem.NewByteArray(input)})
		actual := crypto.bls12381PairingList(nil, []stackitem.Item{points}).Value().(blsPoint)
		expected, err := blsPointPairing(blsPoint{g1Point}, blsPoint{g2Point})
		require.NoError(t, err)
		require.Equal(t, expected, actual)
	})
	t.Run("success case", func(t *testing.T) {
		resGT := crypto.bls12381PairingList(nil, []stackitem.Item{stackitem.NewArray([]stackitem.Item{
			stackitem.NewInterop(blsPoint{point: g1Point}),
			stackitem.NewInterop(blsPoint{point: g2Point}),
			stackitem.NewInterop(blsPoint{point: new(bls12381.G1Affine).Neg(g1Point)}),
			stackitem.NewInterop(blsPoint{point: g2Point}),
		})}).Value().(blsPoint).point.(*bls12381.GT)
		require.True(t, resGT.IsOne())
	})
	t.Run("errors", func(t *testing.T) {
...
func TestBlsSerializersEth(t *testing.T) {
	crypto := newCrypto()
	t.Run("success case g1", func(t *testing.T) {
		for _, p := range []*bls12381.G1Affine{g1Point, {}} {
			...
			input := addPadding(uncompressed[:])
			pi := crypto.bls12381DeserializeEth(nil, []stackitem.Item{stackitem.NewByteArray(input)})
			_, ok := pi.Value().(blsPoint)
			require.True(t, ok)
			bi := crypto.bls12381SerializeEth(nil, []stackitem.Item{pi})
	...
	t.Run("success case g2", func(t *testing.T) {
		for _, p := range []*bls12381.G2Affine{g2Point, {}} {
			uncompressed := p.RawBytes()
			uncompressed[0] &= 0b00011111
	...
	t.Run("deserialize invalid item", func(t *testing.T) {
		require.Panics(t, func() {
			crypto.bls12381DeserializeEth(nil, []stackitem.Item{stackitem.Null{}})
		})
	})
	t.Run("deserialize invalid byteslice length", func(t *testing.T) {
		require.Panics(t, func() {
			crypto.bls12381DeserializeEth(nil, []stackitem.Item{stackitem.NewByteArray([]byte{})})
		})
	})
	t.Run("deserialize setbytes fail", func(t *testing.T) {
		bytes := make([]byte, bls12G1EncodedLength)
		bytes[0] = 0b111 << 5
		require.Panics(t, func() {
			crypto.bls12381DeserializeEth(nil, []stackitem.Item{stackitem.NewByteArray(bytes)})
		})
	})
	t.Run("serialize invalid item", func(t *testing.T) {
		require.Panics(t, func() {
			crypto.bls12381SerializeEth(nil, []stackitem.Item{stackitem.Null{}})
		})
	})
	t.Run("serialize invalid item", func(t *testing.T) {
		require.Panics(t, func() {
	...
	t.Run("serialize invalid interop value", func(t *testing.T) {
		require.Panics(t, func() {
			crypto.bls12381SerializeEth(nil, []stackitem.Item{stackitem.NewInterop(nil)})
		})
	})
	t.Run("serialize invalid point type", func(t *testing.T) {
		require.Panics(t, func() {
...
func TestBlsSerializerEthList(t *testing.T) {
	crypto := newCrypto()
	...
	t.Run("errors", func(t *testing.T) {
		...
		require.PanicsWithValue(t, "even number of elements is required", func() {
			crypto.bls12381SerializeList(nil, []stackitem.Item{
				stackitem.NewArray([]stackitem.Item{
					stackitem.NewInterop(nil),
				}),
		...
		require.PanicsWithValue(t, "invalid sequence of points", func() {
			crypto.bls12381SerializeList(nil, []stackitem.Item{
				stackitem.NewArray([]stackitem.Item{
					stackitem.NewInterop(blsPoint{g1Point}),
					stackitem.NewInterop(blsPoint{g1Point}),
...
func TestDeserializerPoints(t *testing.T) {
	crypto := newCrypto()
	t.Run("success case", func(t *testing.T) {
		input := append(g1, g2...)
		...
		actual := crypto.bls12381DeserializePoints(nil, []stackitem.Item{stackitem.NewByteArray(input)}).Value().([]stackitem.Item)
		require.Equal(t, expected, actual)
	})
	t.Run("errors", func(t *testing.T) {
		require.PanicsWithValue(t, "deserializer requires at least one pair", func() {
			crypto.bls12381DeserializePoints(nil, []stackitem.Item{
...
func TestDeserializerPointScalarPairs(t *testing.T) {
	crypto := newCrypto()
	t.Run("success case", func(t *testing.T) {
		one := make([]byte, fr.Bytes)
		copy(one, bigint.ToPreallocatedBytes(new(big.Int).SetUint64(1), nil))
		...
		actual := crypto.bls12381DeserializeG1PointScalarPairs(nil, []stackitem.Item{stackitem.NewByteArray(input)}).Value().([]stackitem.Item)
		require.Equal(t, expected, actual)
	})
	t.Run("errors", func(t *testing.T) {
		require.PanicsWithValue(t, "deserializer requires at least one pair", func() {
			crypto.bls12381DeserializeG1PointScalarPairs(nil, []stackitem.Item{
				stackitem.NewByteArray(nil),
			})
		})
		require.Panics(t, func() {
