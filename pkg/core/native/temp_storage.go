...
const (
	...
	// msPerYear is the number of milliseconds in one year.
	msPerYear = int64(365 * 24 * time.Hour / time.Millisecond)
)
...
func (s *TempStorage) put(ic *interop.Context, args []stackitem.Item) stackitem.Item {
	...
	recordKey := makeRecordKey(contract.ID, key)
	need := s.calculateStoragePrice(ic, recordKey, value, int64(validTill-ic.Block.Timestamp))
	if err := ic.VM.AddPicoGas(need); err != nil {
		panic(fmt.Errorf("failed to charge temporary storage fee (%d picoGAS): %w", need, err))
	}
...
func (s *TempStorage) calculateStoragePrice(ic *interop.Context, recordKey, value []byte, lifetime int64) int64 {
	return istorage.CalculatePrice(ic, s.ID, recordKey, value) / min(lifetime, msPerYear) * msPerYear
}
...
func (s *TempStorage) getInternal(ic *interop.Context, h util.Uint160, key []byte) (stackitem.Item, uint64) {
	...
	si := ic.DAO.GetStorageItem(s.ID, makeRecordKey(contract.ID, key))
	if si == nil {
...
func (s *TempStorage) findInternal(ic *interop.Context, h util.Uint160, prefix []byte, opts int64) stackitem.Item {
	...
	pref := makeRecordKey(contract.ID, prefix)
...
func (s *TempStorage) renew(ic *interop.Context, args []stackitem.Item) stackitem.Item {
	...
	value := old[8:]
	need := s.calculateStoragePrice(ic, recordKey, value, int64(validTill-oldValidTill))
	if err := ic.VM.AddPicoGas(need); err != nil {
		panic(fmt.Errorf("failed to charge temporary storage fee (%d picoGAS): %w", need, err))
	}
...
func makeRecordKey(contractID int32, key []byte) []byte {
