...
func getTestContractState(bc *Blockchain) (*state.Contract, *state.Contract) {
	...
	addOff := w.Len()
	emit.Opcodes(w, opcode.ADD, opcode.RET)
	addMultiOff := w.Len()
	emit.Opcodes(w, opcode.ADD, opcode.ADD, opcode.RET)
	ret7Off := w.Len()
	emit.Opcodes(w, opcode.PUSH7, opcode.RET)
	dropOff := w.Len()
	emit.Opcodes(w, opcode.DROP, opcode.RET)
	initOff := w.Len()
	emit.Opcodes(w, opcode.INITSSLOT, 1, opcode.PUSH3, opcode.STSFLD0, opcode.RET)
	add3Off := w.Len()
	emit.Opcodes(w, opcode.LDSFLD0, opcode.ADD, opcode.RET)
	invalidRetOff := w.Len()
	emit.Opcodes(w, opcode.PUSH1, opcode.PUSH2, opcode.RET)
	justRetOff := w.Len()
	emit.Opcodes(w, opcode.RET)
	verifyOff := w.Len()
	emit.Opcodes(w, opcode.LDSFLD0, opcode.SUB,
		opcode.CONVERT, opcode.Opcode(stackitem.BooleanT), opcode.RET)
	deployOff := w.Len()
	emit.Opcodes(w, opcode.SWAP, opcode.JMPIF, 2+8+1+1+1+1+39+3)
	emit.String(w, "create")                                  // 8 bytes
	emit.Int(w, 2)                                            // 1 byte
	emit.Opcodes(w, opcode.PACK)                              // 1 byte
	...
	emit.Opcodes(w, opcode.CALL, 3+8+1+1+1+1+39+3, opcode.RET)
	emit.String(w, "update")                                  // 8 bytes
	emit.Int(w, 2)                                            // 1 byte
	emit.Opcodes(w, opcode.PACK)                              // 1 byte
	emit.Int(w, 1)                                            // 1 byte (args count for `serialize`)
	emit.Opcodes(w, opcode.PACK)                              // 1 byte (pack args into array for `serialize`)
	emit.AppCallNoArgs(w, stdHash, "serialize", callflag.All) // 39 bytes
	emit.Opcodes(w, opcode.CALL, 3, opcode.RET)
	putValOff := w.Len()
	emit.String(w, "initial")
	...
	emit.String(w, "initial")
	emit.Syscall(w, interopnames.SystemStorageGetContext)
	emit.Syscall(w, interopnames.SystemStorageGet)
	emit.Opcodes(w, opcode.RET)
	delValOff := w.Len()
	emit.Syscall(w, interopnames.SystemStorageGetContext)
	emit.Syscall(w, interopnames.SystemStorageDelete)
	emit.Opcodes(w, opcode.RET)
	onNEP17PaymentOff := w.Len()
	emit.Syscall(w, interopnames.SystemRuntimeGetCallingScriptHash)
	emit.Int(w, 4)
	emit.Opcodes(w, opcode.PACK)
	emit.String(w, "LastPayment")
	emit.Syscall(w, interopnames.SystemRuntimeNotify)
	emit.Opcodes(w, opcode.RET)
	onNEP11PaymentOff := w.Len()
	emit.Syscall(w, interopnames.SystemRuntimeGetCallingScriptHash)
	emit.Int(w, 5)
	emit.Opcodes(w, opcode.PACK)
	emit.String(w, "LostPayment")
	emit.Syscall(w, interopnames.SystemRuntimeNotify)
	emit.Opcodes(w, opcode.RET)
	update3Off := w.Len()
	emit.Int(w, 3)
	emit.Opcodes(w, opcode.JMP, 2+1)
	updateOff := w.Len()
	emit.Int(w, 2)
	emit.Opcodes(w, opcode.PACK)
	emit.AppCallNoArgs(w, mgmtHash, "update", callflag.All)
	emit.Opcodes(w, opcode.DROP)
	emit.Opcodes(w, opcode.RET)
	destroyOff := w.Len()
	emit.AppCall(w, mgmtHash, "destroy", callflag.All)
	emit.Opcodes(w, opcode.DROP)
	emit.Opcodes(w, opcode.RET)
	invalidStackOff := w.Len()
	emit.Opcodes(w, opcode.NEWARRAY0, opcode.DUP, opcode.DUP, opcode.APPEND) // recursive array
	emit.Syscall(w, interopnames.SystemStorageGetReadOnlyContext)            // interop item
	emit.Opcodes(w, opcode.RET)
	callT0Off := w.Len()
	emit.Opcodes(w, opcode.CALLT, 0, 0, opcode.PUSH1, opcode.ADD, opcode.RET)
	callT1Off := w.Len()
	emit.Opcodes(w, opcode.CALLT, 1, 0, opcode.RET)
	callT2Off := w.Len()
	emit.Opcodes(w, opcode.CALLT, 0, 0, opcode.RET)
	refuelOff := w.Len()
	emit.Opcodes(w, opcode.PUSH2, opcode.PACK)
	emit.AppCallNoArgs(w, bc.contracts.GAS.Hash, "refuel", callflag.States|callflag.AllowNotify)
	emit.Opcodes(w, opcode.DROP)
	burnGasOff := w.Len()
	emit.Syscall(w, interopnames.SystemRuntimeBurnGas)
	emit.Opcodes(w, opcode.RET)
...
func TestRuntimeGetNetwork(t *testing.T) {
	...
	w := io.NewBufBinWriter()
	emit.Syscall(w, interopnames.SystemRuntimeGetNetwork)
