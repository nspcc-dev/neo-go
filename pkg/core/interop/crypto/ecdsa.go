...
func ECDSASecp256r1CheckMultisig(ic *interop.Context) (*fee.InteropRunStats, error) {
	pkeys, err := ic.VM.Estack().PopSigElements()
	if err != nil {
		return nil, fmt.Errorf("wrong key parameters: %w", err)
	}
	sigs, err := ic.VM.Estack().PopSigElements()
	if err != nil {
		return nil, fmt.Errorf("wrong signature parameters: %w", err)
	}
	var stats *fee.InteropRunStats
	if ic.IsHardforkEnabled(config.HFGorgon) {
		stats = &fee.InteropRunStats{EntriesCount: len(pkeys)}
	...
	if len(pkeys) < len(sigs) {
		return nil, errors.New("more signatures than there are keys")
	}
	if ic.IsHardforkEnabled(config.HFGorgon) {
		for i, s := range sigs {
			if len(s) != keys.SignatureLen {
				return nil, fmt.Errorf("wrong signature #%d length: expected %d, got %d", i, keys.SignatureLen, len(s))
			}
		}
	}
...
func ECDSASecp256r1CheckSig(ic *interop.Context) (*fee.InteropRunStats, error) {
	...
	if ic.IsHardforkEnabled(config.HFGorgon) && len(signature) != keys.SignatureLen {
		return nil, fmt.Errorf("wrong signature length: expected %d, got %d", keys.SignatureLen, len(signature))
	}
