...
func ECDSASecp256r1CheckMultisigDummy(ic *interop.Context) error {
	pkeys, err := ic.VM.Estack().PopSigElements()
	if err != nil {
		return fmt.Errorf("wrong key parameters: %w", err)
	}
	...
	message := hash.NetSha256(ic.Network, ic.Container).BytesBE()
	m, n := len(sigs), len(pkeys)
	sigok := true
	for i, j := 0, 0; i < m && j < n; {
		pkey, err := keys.NewPublicKeyFromBytes(pkeys[j], elliptic.P256())
		if err != nil {
			return fmt.Errorf("invalid key #%d: %w", j, err)
		}
		if pkey.Verify(sigs[i], message) {
			i++
		}
		j++
		if m-i > n-j {
