...
const (
	countPoints = 8
	// maxSubitems mirrors transaction's own private maxSubitems limit on
	// Signer.AllowedContracts, Signer.AllowedGroups, Signer.Rules and the
	// width of a single AND/OR condition.
	maxSubitems = 16
)
...
func genContractCallNameLen(nopNs float64) [][]string {
	measure := func(nameLen int) []string {
		...
		return []string{strconv.Itoa(nameLen), "0", "0", "1", "0", strconv.FormatFloat(ns/nopNs, 'f', 6, 64)}
	}
...
func genContractCallArgsCount(nopNs float64) [][]string {
	measure := func(argsCount int) []string {
		...
		return []string{"0", strconv.Itoa(argsCount), "0", "1", "0", strconv.FormatFloat(ns/nopNs, 'f', 6, 64)}
	}
...
func genContractCallPermissionsCount(nopNs float64) [][]string {
	measure := func(permissionsCount int) []string {
		...
		return []string{"0", "0", strconv.Itoa(permissionsCount), "1", "0", strconv.FormatFloat(ns/nopNs, 'f', 6, 64)}
	}
...
func genContractCallMethodsCount(nopNs float64) [][]string {
	measure := func(methodsCount int) []string {
		...
		return []string{"0", "0", "0", strconv.Itoa(methodsCount), "0", strconv.FormatFloat(ns/nopNs, 'f', 6, 64)}
	}
...
func genContractCallRefsDelta(nopNs float64) [][]string {
	measure := func(arraySize int) []string {
		cs := newContractWithParamCount(1)
		inner := make([]any, arraySize)
		ns := measureNsPerOp(func(b *testing.B) {
			benchInterop(b, nil, benchCase{
				f:          contract.Call,
				itemsToAdd: []any{[]any{inner}, int32(0), "", cs.Hash},
				// Call pushes a new context onto Istack.
				// One Step executes single RET and unloads it.
				isStepNeeded: true,
			}, cs)
		})
		return []string{"0", "1", "0", "1", strconv.Itoa(arraySize), strconv.FormatFloat(ns/nopNs, 'f', 6, 64)}
	}
		...
		ns := measureNsPerOp(func(b *testing.B) {
			...
			for b.Loop() {
				...
				ic.VM.LoadNEFMethod(&hfMD.NEF, &hfMD.Manifest, util.Uint160{}, hfMD.Hash,
					callflag.All, false, method.SyscallOffset, -1, nil, nil, false)
				ic.VM.Estack().PushItem(arr)
				ic.VM.Estack().PushVal(0)
...
func genIteratorValueConst(nopNs float64) float64 {
	ch := make(chan storage.KeyValue, 1)
	it := istorage.NewIterator(ch, nil, istorage.FindValuesOnly)
...
func genCurrentSignersCount(nopNs float64) [][]string {
	...
	for i := range rules {
		...
		for j := range top {
			...
			for k := range mid {
				h := random.Uint160()
				csh := transaction.ConditionScriptHash(h)
				mid[k] = &csh
			}
			top[j] = &mid
		}
	...
	measureSigners := func(signers []transaction.Signer) float64 {
		ic := &interop.Context{}
		ic.SpawnVM()
	...
	measure := func(signersCount int) []string {
		signers := make([]transaction.Signer, signersCount)
		for i := range signers {
		...
		return []string{strconv.Itoa(signersCount), strconv.FormatFloat(measureSigners(signers), 'f', 6, 64)}
	}

	stepSignersCount := transaction.MaxAttributes / countPoints
	currSignersCount := stepSignersCount
	rows := make([][]string, 0, countPoints+2)
	lightSigner := []transaction.Signer{{Account: random.Uint160(), Scopes: transaction.CalledByEntry}}
	rows = append(rows, []string{"0", strconv.FormatFloat(measureSigners(lightSigner), 'f', 6, 64)})
	rows = append(rows, measure(1))
	for i := 1; i <= countPoints; i++ {
...
func checkWitnessBenchCase(b *testing.B, groupsCount, rulesCount int, groupPub *keys.PublicKey) {
	blkChain, _ := chain.NewSingle(b)
	ic, err := blkChain.GetTestVM(trigger.Application, &transaction.Transaction{}, &block.Block{})
	if err != nil {
		b.Fatal(err)
	...
	groups := make([]manifest.Group, groupsCount)
	for i := range groups {
		groups[i] = manifest.Group{PublicKey: groupPub}
	}
	cs := &state.Contract{
		ContractBase: state.ContractBase{
			Hash:     targetHash,
			Manifest: manifest.Manifest{Groups: groups},
		},
	}
	...
	rules := make([]transaction.WitnessRule, rulesCount)
	for i := range rules {
		top := make(transaction.ConditionOr, maxSubitems)
		for j := range top {
			mid := make(transaction.ConditionOr, maxSubitems)
	...
	signers[transaction.MaxAttributes-1] = transaction.Signer{
		...
		Scopes: transaction.CalledByEntry | transaction.CustomContracts |
			transaction.CustomGroups | transaction.Rules,
		AllowedContracts: allowedContracts,
		AllowedGroups:    allowedGroups,
		Rules:            rules,
	}
	ic.Tx.Signers = signers

	ic.VM.LoadScriptWithHash([]byte{byte(opcode.RET)}, random.Uint160(), callflag.All)
	ic.VM.LoadScriptWithHash([]byte{byte(opcode.RET)}, random.Uint160(), callflag.All)
	ic.VM.LoadScriptWithHash([]byte{byte(opcode.RET)}, targetHash, callflag.All)

	benchInterop(b, ic, benchCase{
		f:           runtime.CheckWitness,
		itemsToAdd:  []any{checkedHash.BytesBE()},
		amountToPop: 1,
	}, cs)
}

func genCheckWitnessRulesCount(nopNs float64) [][]string {
	measure := func(rulesCount int) []string {
		ns := measureNsPerOp(func(b *testing.B) {
			checkWitnessBenchCase(b, 0, rulesCount, nil)
		})
		return []string{strconv.Itoa(rulesCount), "0", strconv.FormatFloat(ns/nopNs, 'f', 6, 64)}
	}

	stepRulesCount := maxSubitems / countPoints
	currRulesCount := stepRulesCount
	rows := make([][]string, 0, countPoints+2)
	rows = append(rows, []string{"0", "0", strconv.FormatFloat(genCheckWitnessCalledByEntry(nopNs), 'f', 6, 64)})
	rows = append(rows, measure(1))
	for i := 1; i <= countPoints; i++ {
		rows = append(rows, measure(currRulesCount))
		currRulesCount += stepRulesCount
	}
	return rows
}

func getMaxManifestGroupsCount(pub *keys.PublicKey) int {
	i := sort.Search(manifest.MaxManifestSize, func(n int) bool {
		m := manifest.NewManifest("target")
		m.Groups = make([]manifest.Group, n)
		for j := range n {
			m.Groups[j] = manifest.Group{PublicKey: pub, Signature: random.Bytes(64)}
		}
		data, _ := json.Marshal(m)
		return len(data) > manifest.MaxManifestSize
	})
...
func genCheckWitnessGroupsCount(nopNs float64) [][]string {
	groupKey, err := keys.NewPrivateKey()
	if err != nil {
		panic(err)
	}
	groupPub := groupKey.PublicKey()
	maxGroupsCount := getMaxManifestGroupsCount(groupPub)

	measure := func(groupsCount int) []string {
		ns := measureNsPerOp(func(b *testing.B) {
			checkWitnessBenchCase(b, groupsCount, maxSubitems, groupPub)
		})
		return []string{strconv.Itoa(maxSubitems), strconv.Itoa(groupsCount), strconv.FormatFloat(ns/nopNs, 'f', 6, 64)}
	}

	stepGroupsCount := maxGroupsCount / countPoints
	currGroupsCount := stepGroupsCount
	rows := make([][]string, 0, countPoints+1)
	rows = append(rows, measure(0))
	for i := 1; i <= countPoints; i++ {
		rows = append(rows, measure(currGroupsCount))
		currGroupsCount += stepGroupsCount
	}
	return rows
}

func genCheckWitnessCalledByEntry(nopNs float64) float64 {
	ns := measureNsPerOp(func(b *testing.B) {
		blkChain, _ := chain.NewSingle(b)
		ic, err := blkChain.GetTestVM(trigger.Application, &transaction.Transaction{}, &block.Block{})
		if err != nil {
...
func genNotifyEventsCount(nopNs float64) [][]string {
	measure := func(eventsCount int) []string {
		...
		return []string{strconv.Itoa(eventsCount), "0", "0", strconv.FormatFloat(ns/nopNs, 'f', 6, 64)}
	}
...
func genNotifyParamsCount(nopNs float64) [][]string {
	measure := func(paramsCount int) []string {
		...
		return []string{"1", strconv.Itoa(paramsCount), "0", strconv.FormatFloat(ns/nopNs, 'f', 6, 64)}
	}
...
func nestedArrayItem(depth int) stackitem.Item {
...
func getMaxNotifyNestingDepth() int {
	d := dao.NewSimple(storage.NewMemoryStore(), false)
	i := sort.Search(vm.MaxStackSize, func(n int) bool {
		bytes, err := d.GetItemCtx().Serialize(nestedArrayItem(n), false)
		if err != nil {
			return true
		}
		return len(bytes) > runtime.MaxNotificationSize
...
func genNotifyItemsCount(nopNs float64) [][]string {
	params := []manifest.Parameter{manifest.NewParameter("p0", smartcontract.AnyType)}
	m := manifest.NewManifest("caller")
	m.ABI.Events = []manifest.Event{{Name: "target", Parameters: params}}

	measure := func(depth int) []string {
		item := nestedArrayItem(depth)
		...
		ns := measureNsPerOp(func(b *testing.B) {
			...
			benchInterop(b, ic, benchCase{
				f:          runtime.Notify,
				itemsToAdd: []any{item, "target"},
			}, nil)
		})
		return []string{"1", "1", strconv.Itoa(depth), strconv.FormatFloat(ns/nopNs, 'f', 6, 64)}
	}

	maxDepth := getMaxNotifyNestingDepth()
	stepDepth := maxDepth / countPoints
	currDepth := stepDepth
	rows := make([][]string, 0, countPoints+1)
	rows = append(rows, measure(2))
	for i := 1; i <= countPoints; i++ {
		rows = append(rows, measure(currDepth))
		currDepth += stepDepth
	}
	return rows
}
...
var interopGenerators = map[string][]depGenerator{
	...
	"CheckWitness": {
		{"rules_count", genCheckWitnessRulesCount},
		{"groups_count", genCheckWitnessGroupsCount},
	},
	"ContractCall": {
		...
		{"refs_delta", genContractCallRefsDelta},
	},
	"ContractCallNative": {
	...
	"CheckMultisigDummy": {
		{"keys_count", genCheckMultisigDummy},
	},
	"CurrentSigners": {
		{"signers_count", genCurrentSignersCount},
	},
	"IteratorValue": {
	...
	"Notify": {
		...
		{"items_count", genNotifyItemsCount},
	},
}
...
var staticGenerators = map[string]func(nopNs float64) float64{
	...
	"IteratorValueConst":     genIteratorValueConst,
	"BurnGas":                genBurnGas,
	"GasLeft":                genGasLeft,
...
func genCheckMultisigDummy(nopNs float64) [][]string {
	priv, err := keys.NewPrivateKey()
	if err != nil {
		panic(err)
	}
	...
	measure := func(keysCount int) []string {
		...
		ns := measureNsPerOp(func(b *testing.B) {
			benchInterop(b, ic, benchCase{
				f:           crypto.ECDSASecp256r1CheckMultisigDummy,
				itemsToAdd:  []any{sigs, pubs},
				amountToPop: 1,
			}, nil)
		})
