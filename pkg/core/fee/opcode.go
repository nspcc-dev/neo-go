...
var (
	...
	clearItemsW         = []float64{7.42986919, -123.79142442}
	convertArrOrStructW = []float64{23.66101563, 267.30101563}
	convertByteArrayW   = []float64{4.69064884e-01, 1.99078000e+03}
	dropW               = []float64{7.5673601, 157.08139535}
	dupW                = []float64{4.16672700e-01, 5.83711628e+03}
	hasKeyW             = []float64{17.61636991, -263.8021439}
	initSlotW           = []float64{33.9219805, -92.13953488}
	keysW               = []float64{112.97893297, 686.04870041}
	memcpyW             = []float64{4.51368336e-02, 2.69825581e+02}
	newArrayByteOrIntW  = []float64{71.78479288, 1277.08711846}
	...
	pickItemByteArrayW  = []float64{4.98801059e+00, 1.68388041e+01, 4.64470227e-01, 1.52836315e+03}
	popItemW            = []float64{7.56705518, 4.88905775, 178.38691174}
	removeW             = []float64{7.71219314, 7.50630147, 18.67424943, -7.57699564}
	...
	sizeW               = []float64{7.75644985, -54.11627907}
...
func ValuesGas(args ...any) float64 {
	...
	var (
		ar = args[0].(stackitem.Type)
		vr = args[1].(stackitem.Type)
		n  = args[2].(int)
	)
