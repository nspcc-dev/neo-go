...
var (
	appendNotRefW       = []float64{18.26335948, 7.67537453, 5.65574655, 218.2982638}
	appendRefW          = []float64{14.81385293, 5.74464175, 39.92850615}
	...
	convertByteArrayW   = []float64{4.20632121e-01, 1.25778397e+03}
	dropW               = []float64{7.5673601, 157.08139535}
	dupW                = []float64{4.07258562e-01, 1.87688302e+03}
	hasKeyW             = []float64{3.18273914, 17.21451537, 102.97865804}
	...
	packW               = []float64{21.48115007, 917.05813953}
	packMapW            = []float64{8.55493437, 2424.88841189}
	...
	reverseItemsArrayW  = []float64{2.93099731, 7.55422351, 405.71035197}
	reverseItemsBufferW = []float64{2.95663891e-01, 2.02119266e+03}
	...
	stW                 = []float64{6.68182231, 396.53065952}
	substrW             = []float64{3.73879475e-01, 1.51875581e+03}
	unpackRefAnyW       = []float64{7.49327755, 545.93718883}
	unpackNotRefAnyW    = []float64{6.23912533, 545.93718883}
	unpackRefMapW       = []float64{7.49327755, 528.82044153}
	unpackNotRefMapW    = []float64{6.23912533, 528.82044153}
...
func PackMapGas(args ...any) float64 {
	...
	ss := args[0].(int)
	return floats.Dot(packMapW, []float64{float64(ss), 1})
}
...
func UnpackGas(args ...any) float64 {
	...
	case stackitem.ArrayT, stackitem.StructT:
		if isref {
			return floats.Dot(unpackRefAnyW, []float64{float64(l), 1})
		}
		return floats.Dot(unpackNotRefAnyW, []float64{float64(l), 1})
	case stackitem.MapT:
		if isref {
			return floats.Dot(unpackRefMapW, []float64{float64(l), 1})
		}
		return floats.Dot(unpackNotRefMapW, []float64{float64(l), 1})
	default:
