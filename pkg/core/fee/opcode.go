...
var (
	...
	catW                 = []float64{4.56683001e-03, 8.45587495}
	...
	newBuffer            = []float64{3.01582547e-03, -4.97502153e+00}
	packW                = []float64{0.16588031, 7.59248581}
	packMapW             = []float64{0.10561647, 29.93689397}
	...
	popItemW             = []float64{0.09342043, 0.06035874, 2.20230755}
	removeW              = []float64{0.09338122, 0.09080587, 0.2270293, 2.28118475}
	reverseItemsArrayW   = []float64{0.03618515, 0.09326202, 5.00876978}
	reverseItemsBufferW  = []float64{3.65017149e-03, 2.49529957e+01}
	reverseW             = []float64{0.01799757, 2.31099212}
	rollW                = []float64{0.00547301, 1.69196813}
	setitemW             = []float64{0.09236674, 0.08812606, 0.15259838, 0.45037297, 5.64765264}
	...
	valuesW              = []float64{0.28806486, 0.3421585, 19.92522492}
	xDropW               = []float64{0.09456698, 0.00571582, 1.75963926}
)
...
func PackMapGas(args ...any) float64 {
	...
	nc := args[0].(int)
	return floats.Dot(packMapW, []float64{float64(nc), 1})
}
