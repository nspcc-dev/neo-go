...
func (bc *Blockchain) tryRunGC(oldHeight uint32) time.Duration {
	...
	if tgtBlock > int64(bc.config.Ledger.GarbageCollectionPeriod) && newHeight != oldHeight {
		...
		dur += bc.removeUntraceableBlocks(oldHeight, newHeight, uint32(tgtBlock)) // TODO: here it's not correct to pass oldHeight; we need to pass the previous value of targetBlock instead to support full removal.
...
func (bc *Blockchain) removeUntraceableBlocks(oldHeight, newHeight, tgtHeight uint32) time.Duration {
	if newHeight/headerBatchCount == tgtHeight/headerBatchCount { // current batch of header hashes is not stored yet.
	...
	bc.log.Info("starting untraceable blocks garbage collection", zap.Uint32("from", oldHeight), zap.Uint32("to", tgtHeight))
	var (
		start   = time.Now()
		kvcache = bc.dao.GetPrivate()
		removed int
	)
	for index := oldHeight; index < tgtHeight; index++ {
		_, err := kvcache.DeleteBlock(bc.GetHeaderHash(index))
		if err != nil {
