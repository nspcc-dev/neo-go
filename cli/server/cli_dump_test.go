...
func TestDBDumpRestoreIncremental(t *testing.T) {
	...
	e.Run(t, append(restoreBaseArgs, "--in", incDump, "-n", "--count", "5")...)

	// Checking the auto-detection of the incremental dump
	e.Run(t, append(restoreBaseArgs, "--in", incDump, "--count", "10")...)
}
