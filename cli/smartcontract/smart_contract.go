...
func NewCommands() []*cli.Command {
	...
	updateFlags := append([]cli.Flag{
		...
		&cli.BoolFlag{
			Name:  "strict",
			Usage: "Disables fallback call when data is not provided",
		},
	}, invokeFunctionFlags...)
...
func contractUpdate(ctx *cli.Context) error {
	...
	// If that fails (and --strict isn’t set), fall back to the original format
	// which expects only (script, manifest) with no extra data.
	if err == nil || ctx.Bool("strict") || !strings.Contains(err.Error(), "System.Contract.Call failed: method not found: update/3.") {
		return err
	}
